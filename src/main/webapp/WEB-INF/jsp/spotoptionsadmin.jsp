...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-8 col-lg-6">
		...
		<form method="POST" action="spotadminoptions" novalidate="novalidate" role="form">
                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
			<div class="form-group">
				<label for="userID">Bedrijven die niet gekozen hebben: </label>
				<select class="form-control" name="userID" id="userId">
					<c:forEach var="freeUser" items="${freeUsers.id}">
						<option value="<c:out value="${freeUser.id}"/>"><c:out value="${freeUser.id}"/></option>
					</c:forEach>
				</select>
			</div>
			<fieldset>
